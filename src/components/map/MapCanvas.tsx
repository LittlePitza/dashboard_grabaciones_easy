...
// Espera a que window.L esté disponible (Leaflet carga desde CDN con defer)
function waitForLeaflet(cb: (L: any) => void, tries = 0) {
  const L = (window as any).L;
  if (L) { cb(L); return; }
  if (tries > 30) return; // máx ~3s
...
export default function MapCanvas({ locations }: Props) {
  ...
  // Inicializar mapa — esperar Leaflet
  useEffect(() => {
  ...
  // Renderizar markers cuando el mapa esté listo y cambien las locaciones
  useEffect(() => {
    ...
    locations.forEach(loc => {
      ...
      const statusLabel = st === 'never' ? 'Sin grabar'
        : st === 'overdue' ? `Venció hace ${Math.abs(due!)}d`
        : st === 'soon'    ? `Vence en ${due}d`
        : `Al corriente · ${due}d rest.`;
        ...
        <div style="padding:12px 14px;min-width:190px;font-family:system-ui,sans-serif">
          ...
          <div style="display:flex;gap:6px;margin-top:10px;padding-top:8px;border-top:1px solid #e5e7eb">
            <a href="${mapsUrl(loc.lat!, loc.lng!)}" target="_blank"
              style="flex:1;text-align:center;padding:5px 8px;background:#3b82f6;color:#fff;border-radius:6px;font-size:11px;font-weight:600;text-decoration:none">
              Abrir Maps
            </a>
          </div>
        </div>`;
    ...
    // Invalidar tamaño después de renderizar (necesario cuando el contenedor cambia visibilidad)
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      {!ready && (
        <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:'var(--color-surface-offset)', zIndex:1 }}>
